...
local scrlocker    = os.getenv("SCREENLOCKER")
...
local myawesomemenu = {
    ...
    { "new theme", function() run_once({"~/.setwp"}) end },
    ...
    awful.key({ modkey,           }, "Return", function () 
        ...
        awful.spawn(
            ...
            function(out)
                ...
                local myx = tonumber(myx)
                local myy = tonumber(myy)
                local mywidth = tonumber(mywidth)
                local myheight = tonumber(myheight)
                if mywidth==nil or myheight==nil or myx==nil or myy==nil or mywidth==0 or myheight==0 then
                    awful.spawn(terminal)
                    return
                end
                awful.spawn(terminal, {
...
gears.timer({
	...
	autostart=true,
	callback=function()
		run_once({"loginctl lock-session"})
	end,
	single_shot=true
})
